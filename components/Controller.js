...
export default function Controller({onNext, onPrv}) {
  ...
  const isPlaying = useRef('playing'); //paused play loading
  ...
  const returnPlayBtn = () => {
    switch (isPlaying.current) {
      case 'playing':
        return <Icon color="#4c534d" name="pause" size={45} />;
      case 'paused':
        return <Icon color="#4c534d" name="play-arrow" size={45} />;
      default:
  ...
  return (
    <View style={styles.container}>
      <TouchableOpacity onPress={onPrv}>
        <Icon color="#4c534d" name="skip-previous" size={45} />
      </TouchableOpacity>
      <TouchableOpacity onPress={onPlayPause}>
        {returnPlayBtn()}
      </TouchableOpacity>
      <TouchableOpacity onPress={onNext}>
        <Icon color="#4c534d" name="skip-next" size={45} />
      </TouchableOpacity>
    </View>
  );
}
